...
Proof.
  intros. apply hoare_consequence_pre with (P' := fun st : state => parity (st X) = parity m).
  eapply hoare_consequence_post.  apply hoare_while.
  apply hoare_consequence_pre with (P' := fun st : state => parity (st X - 2) = parity m).
  eapply hoare_consequence_pre. apply hoare_asgn.
  unfold assn_sub, update, assert_implies. intros; simpl. assumption.
  unfold assert_implies. intros; simpl. destruct H. 
  rewrite parity_ge_2. assumption. 
  unfold beval in H0. simpl (aeval st (ANum 2)) in H0. simpl (aeval st (AId X)) in H0. apply ble_nat_true in H0. assumption. 
  unfold assert_implies. intros; simpl. destruct H. 
  rewrite <- parity_lt_2 with (st X). assumption.
  unfold beval in H0. simpl (aeval st (ANum 2)) in H0. simpl (aeval st (AId X)) in H0. apply ble_nat_false in H0. assumption.
